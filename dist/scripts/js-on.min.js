...
const containerProduct=document.getElementById("ad");const more=document.getElementById("showMore");async function getResponse(){let link='../data/data-product.json'
fetch(link).then(response=>{return response.json();}).then(data=>{buildElem(data);buildElemAddit(data);more.addEventListener('click',(e)=>{e.preventDefault();buildElem(data);buildElemAddit(data);})}).catch(err=>{console.error(err)});}
getResponse()
function buildElem(values){for(let i=0;i<8;i++){containerProduct.insertAdjacentHTML("afterbegin",`<div class="products-list__elem">
      ...
      `);}}
function buildElemAddit(values){for(let i=0;i<8;i++){if(values[i].seasonOld){const oldPrice=document.querySelectorAll(".products-list__price");const cont=document.querySelectorAll(".products-list__elem")
