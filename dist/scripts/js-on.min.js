...
function buildElemAddit(values){for(let i=start;i<values.length;i++){const cont=document.querySelectorAll(".products-list__elem");const oldPrice=document.querySelectorAll(".products-list__price");if(i<valueId){if(values[i].seasonOld){oldPrice[i].insertAdjacentHTML("beforeend",`
          ...
          `);}}}}
function loadCart(data){containerProduct.onclick=function(event){event.preventDefault();if(event.target.classList.contains('product-list-hover__add')){let idEleme=event.target.dataset.id;for(let i=0;i<data.length;i++){if(data[i].id===idEleme){localStorage.setItem('objElem',JSON.stringify(data[i]))}}}}}